#!/usr/bin/env python3
import os
import re
import glob
import math
import numpy as np
import pandas as pd
from typing import Optional, Tuple, List, Dict

# -------- CONFIG --------
CSV_DIR   = "csvs/cross_temp"   # parent folder containing subfolders of CSVs
TIME_COL  = "Time Elapsed (hours)"
BATT_COL  = "batt_mV"

# Fault columns to consider (same set as your plotting script)
FAULT_COLS = [
    "fault_brownout",
    "fault_sound",
    "fault_sound_brownout",
    "fault_bolt",
    "fault_bolt_brownout",
]

# Training params
RANDOM_STATE = 42
MAX_ITER     = 5000               # give the MLP time to converge
HIDDEN       = (32, 32)           # small MLP is plenty for 1-D mapping
VAL_FRAC     = 0.15               # just for a quick check
MIN_SAMPLES_PER_TEMP = 40         # skip temps with too little data

# ------------------------

from sklearn.neural_network import MLPRegressor
from sklearn.preprocessing import StandardScaler
from sklearn.pipeline import Pipeline
from sklearn.model_selection import train_test_split


def parse_temperature_from_folder(name: str) -> Optional[float]:
    """
    Parse the temperature from the folder name by splitting on '-' and taking the second part.
    Handles cases like 'minus20' as -20. Returns None if not parseable.
    """
    parts = name.split('-')
    if len(parts) < 2:
        return None
    temp_str = parts[1].lower()
    if temp_str.startswith("minus"):
        try:
            return -float(temp_str.replace("minus", ""))
        except Exception:
            return None
    try:
        return float(temp_str)
    except Exception:
        return None


def first_fault_label(df: pd.DataFrame) -> Optional[pd.Index]:
    """
    Returns the index *label* of the first fault, after masking out the first 10% of rows.
    Returns None if no fault found or no fault columns present.
    """
    present_fault_cols = [c for c in FAULT_COLS if c in df.columns]
    if not present_fault_cols:
        return None
    any_fault = df[present_fault_cols].any(axis=1)
    if not any_fault.any():
        return None

    # Mask first 10% of rows
    n_rows = len(df)
    min_idx = int(np.ceil(0.10 * n_rows))
    if min_idx > 0:
        any_fault.iloc[:min_idx] = False

    if not any_fault.any():
        return None

    return any_fault.idxmax()


def collect_voltage_soc_pairs(csv_path: str) -> Optional[pd.DataFrame]:
    """
    From a single CSV, produce rows of (voltage_mV, soc) between t0 and the first fault.
    SoC is defined by time-to-fault normalization.
    """
    try:
        df = pd.read_csv(csv_path)
    except Exception:
        return None

    if TIME_COL not in df.columns or BATT_COL not in df.columns:
        return None

    idx_fault = first_fault_label(df)
    if idx_fault is None:
        return None

    t_series = df[TIME_COL]
    t0 = float(t_series.iloc[0])
    t_fault = float(t_series.loc[idx_fault])
    if not np.isfinite(t0) or not np.isfinite(t_fault) or t_fault <= t0:
        return None

    # Keep samples only up to (and including) first fault row
    # Convert to positional index to slice safely
    try:
        pos_fault = df.index.get_indexer([idx_fault])[0]
    except Exception:
        return None
    df_use = df.iloc[:pos_fault + 1].copy()

    # Compute SoC by time normalization: SoC(t) = (t_fault - t)/(t_fault - t0)
    t = df_use[TIME_COL].astype(float)
    soc = (t_fault - t) / (t_fault - t0)

    # Clean voltage; keep finite rows only
    v = df_use[BATT_COL].astype(float)
    mask = np.isfinite(v) & np.isfinite(soc)
    v = v[mask].values.reshape(-1, 1)  # shape (N,1)
    s = soc[mask].values               # shape (N,)

    if len(s) < 5:
        return None

    return pd.DataFrame({"voltage_mV": v.flatten(), "soc": s})


def aggregate_by_temperature(root_dir: str) -> Dict[str, Dict[str, object]]:
    """
    Returns a dict keyed by temperature key (string),
    each value contains:
      - "temp_value": Optional[float] mode temperature from all CSVs in the folder
      - "data": DataFrame with columns ["voltage_mV", "soc"]
      - "vmin": float, "vmax": float in that temp bucket
    """
    out: Dict[str, Dict[str, object]] = {}
    subfolders = sorted(
        d for d in os.listdir(root_dir)
        if os.path.isdir(os.path.join(root_dir, d))
    )

    for sf in subfolders:
        folder_path = os.path.join(root_dir, sf)
        csv_files = sorted(glob.glob(os.path.join(folder_path, "*.csv")))
        if not csv_files:
            continue

        rows = []
        temps = []
        for p in csv_files:
            pairs = collect_voltage_soc_pairs(p)
            if pairs is not None:
                rows.append(pairs)
                # Try to extract temperature from CSV if present
                try:
                    df = pd.read_csv(p)
                    # Try common temperature column names
                    for col in ["temperature", "temp", "Temperature", "Temp"]:
                        if col in df.columns:
                            temp_vals = pd.to_numeric(df[col], errors="coerce").dropna()
                            if not temp_vals.empty:
                                temps.extend(temp_vals.tolist())
                                break
                except Exception:
                    pass

        if not rows:
            continue

        df_all = pd.concat(rows, ignore_index=True)
        df_all = df_all.dropna()
        df_all = df_all.sort_values("voltage_mV")
        df_all = df_all.reset_index(drop=True)

        if len(df_all) < MIN_SAMPLES_PER_TEMP:
            continue

        vmin = float(np.nanmin(df_all["voltage_mV"].values))
        vmax = float(np.nanmax(df_all["voltage_mV"].values))

        # Determine mode temperature if possible, else fallback to folder name parsing
        temp_value = None
        if temps:
            temps_arr = np.array(temps)
            # Use mode, fallback to mean if all unique
            vals, counts = np.unique(temps_arr, return_counts=True)
            if len(vals) > 0:
                mode_idx = np.argmax(counts)
                temp_value = float(vals[mode_idx])
        if temp_value is None:
            temp_value = parse_temperature_from_folder(sf)

        out[sf] = {
            "temp_value": temp_value,
            "data": df_all,
            "vmin": vmin,
            "vmax": vmax,
        }

    return out


def fit_voltage_to_soc_nn(df: pd.DataFrame) -> Pipeline:
    """
    Fit an MLPRegressor: voltage_mV -> SoC.
    Returns a scikit-learn Pipeline(Scaler -> MLP).
    """
    X = df[["voltage_mV"]].values
    y = df["soc"].values

    # (Optional) quick train/val split to avoid crazy overfit; not strictly needed
    Xtr, Xval, ytr, yval = train_test_split(
        X, y, test_size=VAL_FRAC, random_state=RANDOM_STATE, shuffle=True
    )

    pipe = Pipeline([
        ("scaler", StandardScaler()),
        ("mlp", MLPRegressor(
            hidden_layer_sizes=HIDDEN,
            activation="relu",
            random_state=RANDOM_STATE,
            max_iter=MAX_ITER,
            n_iter_no_change=50,
            learning_rate_init=1e-3,
            early_stopping=True,
            validation_fraction=0.15,
            tol=1e-5,
        )),
    ])
    pipe.fit(Xtr, ytr)

    # You could print a tiny sanity check, but we'll keep output clean.
    return pipe


def find_voltage_for_soc(model: Pipeline, soc_target: float, v_lo: float, v_hi: float,
                         max_iter: int = 60, tol: float = 1e-3) -> Optional[float]:
    """
    Invert the NN by 1-D root finding on f(V) = model(V) - soc_target using bisection.
    Assumes monotonic-ish mapping voltage -> SoC within [v_lo, v_hi].
    Returns None if no sign change (i.e., target outside model range).
    """
    def f(v):
        pred = model.predict(np.array(v, ndmin=2).reshape(-1, 1))[0]
        # clip to [0,1] just in case
        return float(np.clip(pred, 0.0, 1.0) - soc_target)

    flo = f(v_lo)
    fhi = f(v_hi)

    # If not bracketing, try to expand a bit
    span = (v_hi - v_lo)
    if flo * fhi > 0:
        # expand bounds by 2% spans a few times
        for _ in range(5):
            v_lo = v_lo - 0.02 * span
            v_hi = v_hi + 0.02 * span
            flo = f(v_lo)
            fhi = f(v_hi)
            if flo * fhi <= 0:
                break

    if flo * fhi > 0:
        return None  # cannot bracket target

    lo, hi = v_lo, v_hi
    for _ in range(max_iter):
        mid = 0.5 * (lo + hi)
        fmid = f(mid)
        if abs(fmid) < tol:
            return mid
        if flo * fmid <= 0:
            hi = mid
            fhi = fmid
        else:
            lo = mid
            flo = fmid
    return 0.5 * (lo + hi)


def main():
    if not os.path.isdir(CSV_DIR):
        print(f"Missing directory: {CSV_DIR}")
        return

    buckets = aggregate_by_temperature(CSV_DIR)
    if not buckets:
        print(f"No usable data found under {CSV_DIR}")
        return

    # Sort temperatures nicely (numeric when available, otherwise by name)
    def sort_key(item):
        name, payload = item
        tv = payload["temp_value"]
        return (0, tv) if tv is not None else (1, name)

    print(f"Found {len(buckets)} temperature buckets with sufficient data.\n")
    print("Voltage at target SoC by temperature")
    print("===================================")
    print(f"{'Temp':>10} | {'V@10% SoC (mV)':>16} | {'V@1% SoC (mV)':>15} | {'Samples':>7}")
    print("-" * 58)

    for name, payload in sorted(buckets.items(), key=sort_key):
        df = payload["data"]
        vmin = payload["vmin"]
        vmax = payload["vmax"]

        model = fit_voltage_to_soc_nn(df)

        v_at_10 = find_voltage_for_soc(model, soc_target=0.10, v_lo=vmin, v_hi=vmax)
        v_at_01 = find_voltage_for_soc(model, soc_target=0.01, v_lo=vmin, v_hi=vmax)

        # pretty temperature label
        label = f"{payload['temp_value']}" if payload["temp_value"] is not None else name

        v10_str = f"{v_at_10:.2f}" if v_at_10 is not None else "—"
        v01_str = f"{v_at_01:.2f}" if v_at_01 is not None else "—"
        print(f"{label:>10} | {v10_str:>16} | {v01_str:>15} | {len(df):>7}")

    print("\nNotes:")
    print("- SoC is inferred from time-to-first-fault within each CSV (normalized 1→0).")
    print("- We train one NN per temperature bucket (subfolder) to learn SoC ≈ f(voltage).")
    print("- Voltages for 10% and 1% SoC are found by inverting the NN via bisection.")


if __name__ == "__main__":
    main()
